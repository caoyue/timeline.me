...
def _http_call(the_url, method, authorization, **kw):
    ...
    try:
        ...
        if hasattr(r, 'error_code'):
            raise APIError(
                r.error_code, r.get('error', ''), r.get('request', ''))
        return r
...
class APIClient(object):
    ...
    def _parse_access_token(self, r):
        '''
        new:return access token as a JsonDict:
            {"access_token":"your-access-token","expires_in":12345678,"uid":1234},
             expires_in is represented using standard unix-epoch-time
        '''
